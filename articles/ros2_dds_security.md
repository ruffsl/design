...
This can be specified by setting the `ROS_SECURITY_KEYSTORE` environment variable to point to the root of the keystore directory tree, and then specifying the enclave path using the `--ros-args` runtime argument `--security-enclave`, e.g.:
...
Note that this setting takes precedence over `ROS_SECURITY_KEYSTORE` with `--security-enclave`.
